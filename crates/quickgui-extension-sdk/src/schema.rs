...
mod tests {
    ...
    fn list(count: usize, estimate: f32, overscan: usize) -> List {
        List {
            key: 1,
            renderer: 1,
            count,
    ...
    fn frame_with_list(list: List) -> Frame {
        Frame {
            nodes: vec![Node {
                list: Some(list),
                ..Node::default()
    ...
    #[test]
    fn accepts_a_finite_positive_list_and_rejects_out_of_range_metrics() {
        assert!(frame_with_list(list(8, 24.0, 4)).validate().is_ok());
        assert_eq!(
            frame_with_list(list(1_000_001, 24.0, 4)).validate(),
            Err("invalid component list")
        );
        assert_eq!(
            frame_with_list(list(8, f32::INFINITY, 4)).validate(),
            Err("invalid component list")
        );
        assert_eq!(
            frame_with_list(list(8, 0.0, 4)).validate(),
            Err("invalid component list")
        );
        assert_eq!(
            frame_with_list(list(8, 24.0, 129)).validate(),
            Err("invalid component list")
        );
        assert!(
            Frame {
