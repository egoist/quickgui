...
`quickgui build` also writes `<Name>-<version>-linux-<arch>.tar.gz`, `install.sh`, and `latest-linux.txt`. `--upload` publishes all three, and with GitHub the script has a stable URL: `curl -fsSL https://github.com/<owner>/<repo>/releases/latest/download/install.sh | sh` installs into `~/.local/<package>.app` without root, adds the app to the applications menu, and that install keeps itself current. Set `linux.tarball = false` to skip it.
